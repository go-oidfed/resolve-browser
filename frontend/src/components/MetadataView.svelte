    ...
    <div class="space-y-3">
      {#each Object.entries(metadata) as [type, data]}
        <div class="border rounded-lg overflow-hidden">
          ...
          >
            <span class="font-medium text-gray-900">{formatType(type)}</span>
            <svg
              class="w-5 h-5 text-gray-500 transform transition-transform {expandedSections.has(type) ? 'rotate-180' : ''}"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          {#if expandedSections.has(type)}
            <div class="p-3 bg-white">
              <pre class="text-xs bg-gray-50 p-3 rounded overflow-x-auto max-h-64 overflow-y-auto"><code>{JSON.stringify(data, null, 2)}</code></pre>
            </div>
          {/if}
        </div>
      {/each}
